...
use "$datapath/Complete_IPC3_benchmark_adaptation_2026.dta", clear
...
duplicates drop
compress
save "$patstpath/adaptation/Complete_3digits_adaptation_benchmark.dta", replace
...
* All innovation using docdb (benchmark set)
use "$patstpath/adaptation/Complete_3digits_adaptation_benchmark.dta", clear
* Define High Value Inventions (HVI) as patents filed in at least 2 countries
gen byte HVI = (famsize_offices_EPCgrants_docdb>1)
* Is the patented technology (docdb) granted ?
egen GR = sum(granted), by(docdb_family_id technology)
...
save "$patstpath/adaptation/Complete_3digits_ALL_inventions_by_invtcountry_year", replace
...
duplicates drop
compress
save "$patstpath/adaptation/Complete_3digits_ALL_patents_invt_auth_year.dta", replace
...
drop _m
save "$datapath/databases/CCAT_tech_invt_ctry_year", replace

* Expand dataset and set missing values to zeros.
use "$datapath/databases/CCAT_tech_invt_ctry_year", clear
summ publn_year
local min =r(min)
local max =r(max)
...
mmerge invt_iso technology publn_year using "$datapath/databases/CCAT_tech_invt_ctry_year", unmatched(master)
...
* ==== BRICS flag (founding members) ===
gen byte brics_invt = inlist(invt_iso, "BRA", "RUS", "IND", "CHN", "ZAF")
...
save "$datapath/Final_database/adaptation_inv_tech_inventor_ctry_year", replace
...
drop _m
compress
save "$datapath/databases/CCAT_tech_invt_auth_publn_year", replace

* Expand dataset and set missing values to zeros.
use "$datapath/databases/CCAT_tech_invt_auth_publn_year", clear
summ publn_year
local min =r(min)
local max =r(max)
local nb_years =`max' -`min' + 1
...
mmerge invt_iso technology patent_office_iso publn_year using ///
 "$datapath/databases/CCAT_tech_invt_auth_publn_year", unmatched(master)
...
* ==== BRICS flags (founding members) ===
gen byte brics_invt = inlist(invt_iso, "BRA", "RUS", "IND", "CHN", "ZAF")
gen byte brics_office = inlist(patent_office_iso, "BRA", "RUS", "IND", "CHN", "ZAF")
...
save "$datapath/Final_database/adaptation_transfers_tech_inventor_ctry_office_year", replace
