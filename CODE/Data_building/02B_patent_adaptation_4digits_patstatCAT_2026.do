...
*
*						*== PATENT DATABASE USING BENCHMARK 4 DIGITS (Y02A)
*
*=== Objective:
* - Add information to selected patents (Adaptation and benchmark)
* - Build final databases for invention and transfers
*
...
duplicates drop
compress
save "$patstpath/adaptation/all_adaptation_patents_info", replace

* Complete Benchmark (4 digits)
use "$datapath/Complete_IPC4_benchmark_adaptation_2026.dta", clear
* info on all patents in the benchmark (filing and publication year, application authority)
mmerge appln_id using "$patstpath/general/appln_info", unmatched(none) ///
ukeep(appln_auth appln_year earliest_filing_year earliest_publn_year ipr_type granted)
drop _m
...
duplicates drop
compress
save "$patstpath/adaptation/Complete_4digits_adaptation_benchmark.dta", replace
...
use "$patstpath/adaptation/all_adaptation_patents_info", clear
...
egen GR = sum(granted), by(docdb_family_id technology)
...
keep technology docdb_family_id publn_year invt_country HVI granted_docdb
duplicates drop
gen byte x = 1
* Identify the number of inventors of this technology
bysort docdb_family_id technology : egen nb_invt = sum(x)
...
compress
save "$patstpath/adaptation/Adaptation_tech_inventor_ctry_year", replace

* All innovation using docdb (all technologies)
use "$patstpath/adaptation/Complete_4digits_adaptation_benchmark.dta", clear
...
gen byte x = 1
* Identify the number of inventors of this technology
bysort technology  docdb_family_id: egen nb_invt = sum(x)
gen invnbinv=1/nb_invt
* Count number of patents invented per year per country per technology fields
...
save "$patstpath/adaptation/Complete_4digits_ALL_inventions_by_invtcountry_year", replace
...
use "$patstpath/adaptation/all_adaptation_patents_info", clear
keep technology docdb_family_id earliest_publn_year invt_country appln_auth appln_id granted
egen GR = sum(granted), by(docdb_family_id technology appln_auth)
* Is the patented technology (docdb) granted ?
gen granted_docdb = (GR>0)
drop GR granted
duplicates drop
...
gen byte x = 1
* Identify the number of inventors of this technology
bysort docdb_family_id technology appln_auth : egen nb_invt = sum(x)
gen invnbinv=1/(nb_invt)
keep technology docdb_family_id publn_year appln_auth invt_country invnbinv nb_invt granted_docdb
duplicates drop
compress
save "$patstpath/adaptation/all_adaptation_patents_marketdataset", replace

use "$patstpath/adaptation/all_adaptation_patents_marketdataset", clear
* Count number of patents transferred between inventor country and recipient country
bysort technology appln_auth invt_country publn_year: egen nb_pat_CCMT = sum(invnbinv)
bysort technology appln_auth invt_country publn_year: egen nb_pat_granted_CCMT = sum(invnbinv*granted_docdb)
keep technology appln_auth invt_country publn_year nb_pat_CCMT  nb_pat_granted_CCMT
duplicates drop
compress
save "$patstpath/adaptation/adaptation_patents_invt_auth_publn_year", replace

* Benchmark : Patents by authority by invt country by year
use "$patstpath/adaptation/Complete_4digits_adaptation_benchmark.dta", clear
keep appln_id earliest_publn_year invt_country appln_auth technology famsize_offices_EPCgrants_docdb docdb_family_id granted
* Is the patented technology (docdb) granted ?
egen GR = sum(granted), by(docdb_family_id technology appln_auth)
gen granted_docdb = (GR>0)
drop GR granted
duplicates drop
* Add detailed information on countries where the patent has been filed
merge appln_id using "$patstpath/general/PRS_EPO_national_phase", unmatched(master) keep(country)
replace appln_auth = country if country!=""
drop country
* Define the publication year of the invention as the earliest publication year within the family
bys docdb_family_id : egen publn_year=min(earliest_publn_year)
...
duplicates drop
compress
save "$patstpath/adaptation/Complete_4digits_ALL_patents_invt_auth_year.dta", replace
...
* Inventions
use "$patstpath/adaptation/Adaptation_tech_inventor_ctry_year", clear
order technology invt_country publn_year
sort technology invt_country publn_year
mmerge invt_country publn_year technology using ///
 "$patstpath/adaptation/Complete_4digits_ALL_inventions_by_invtcountry_year"
drop _m
...
drop _m
save "$datapath/databases/CCAT_4digits_tech_invt_ctry_year_Induced", replace

* Expand dataset and set missing values to zeros.
use "$datapath/databases/CCAT_4digits_tech_invt_ctry_year_Induced", clear
summ publn_year
local min =r(min)
local max =r(max)
...
mmerge invt_iso technology publn_year using ///
 "$datapath/databases/CCAT_4digits_tech_invt_ctry_year_Induced", unmatched(master)
...
gen tech_name =""
replace tech_name ="Adaptation" if technology=="Y02A"
...
mmerge invt_iso using "$otherpath/income_region_World_Bank_2025", unmatched(master) ukeep(income)

drop if income ==""
drop if invt_name =="Czechia"

save "$datapath/Final_database/adaptation_inv_tech_inventor_ctry_year_4digits", replace

* Transfers
use "$patstpath/adaptation/adaptation_patents_invt_auth_publn_year", clear
keep if publn_year>1969 & publn_year<2026
mmerge technology appln_auth publn_year invt_country using ///
 "$patstpath/adaptation/Complete_4digits_ALL_patents_invt_auth_year.dta"
drop _m
ren invt_country iso2
mmerge iso2 using "$otherpath/CountryID", unmatched(none) ukeep(iso3)
ren iso3 invt_iso
...
drop _m
compress
save "$datapath/databases/CCAT_4digits_tech_invt_auth_publn_year_Induced", replace

use "$datapath/databases/CCAT_4digits_tech_invt_auth_publn_year_Induced", clear
summ publn_year
local min =r(min)
local max =r(max)
local nb_years =`max' -`min' + 1
...
mmerge invt_iso technology patent_office_iso publn_year using ///
 "$datapath/databases/CCAT_4digits_tech_invt_auth_publn_year_Induced", unmatched(master)
...
mmerge invt_iso using "$otherpath/income_region_World_Bank_2025", unmatched(master) ukeep(income region)
rename income income_invt
rename region region_invt
drop _merge

* == Income group for recipient country: creation of income_office ==
mmerge patent_office_iso using "$otherpath/income_region_World_Bank_2025", unmatched(master) ukeep(income region)
...
save "$datapath/Final_database/adaptation_transfers_tech_inventor_ctry_office_year_4digits", replace
